...
export class VSCodeUtils {
   ...
   * 安全的用戶輸入
   */
  static async getUserInput(options: InputOptions): Promise<string | undefined> {
    const result = await ErrorHandlingUtils.executeWithErrorHandling(
      async () => {
   ...
   * 快速選擇
   */
  static async showQuickPick<T extends vscode.QuickPickItem>(options: QuickPickOptions): Promise<T | undefined> {
    const result = await ErrorHandlingUtils.executeWithErrorHandling(
      async () => {
   ...
   * 安全執行命令
   */
  static async executeCommand<T = any>(command: string, ...args: any[]): Promise<T | undefined> {
    const result = await ErrorHandlingUtils.executeWithErrorHandling(
   ...
   * 註冊命令
   */
  static registerCommand(command: string, callback: (...args: any[]) => any, context: vscode.ExtensionContext): void {
  ...
  static async showSaveDialog(options?: vscode.SaveDialogOptions): Promise<vscode.Uri | undefined> {
    const result = await ErrorHandlingUtils.executeWithErrorHandling(
      () => Promise.resolve(vscode.window.showSaveDialog(options)),
      '顯示保存對話框',
      { logError: true, showToUser: false }
    );
  ...
  static async showOpenDialog(options?: vscode.OpenDialogOptions): Promise<vscode.Uri[] | undefined> {
    const result = await ErrorHandlingUtils.executeWithErrorHandling(
      () => Promise.resolve(vscode.window.showOpenDialog(options)),
      '顯示打開對話框',
      { logError: true, showToUser: false }
    );
