...
export class FileOperationUtils {
  ...
  static async safeFileOperation<T>(
    ...
    const result = await ErrorHandlingUtils.executeWithErrorHandling(() => operation(uri), operationContext, {
      logError: true,
      showToUser: false
    });
  ...
  static async exists(uri: vscode.Uri): Promise<boolean> {
    const result = await this.safeFileOperation(
      uri,
      async uri => {
   ...
   * 安全寫入文件
   */
  static async writeFile(uri: vscode.Uri, content: string, options: FileOperationOptions = {}): Promise<boolean> {
   ...
   * 安全複製文件
   */
  static async copyFile(source: vscode.Uri, target: vscode.Uri, options: FileOperationOptions = {}): Promise<boolean> {
    const result = await ErrorHandlingUtils.executeWithErrorHandling(
      async () => {
        // 確保目標目錄存在
        if (options.createDirectories) {
   ...
   * 安全移動文件
   */
  static async moveFile(source: vscode.Uri, target: vscode.Uri, options: FileOperationOptions = {}): Promise<boolean> {
    const result = await ErrorHandlingUtils.executeWithErrorHandling(
      async () => {
        // 確保目標目錄存在
        if (options.createDirectories) {
   ...
   * 安全刪除文件
   */
  static async deleteFile(uri: vscode.Uri, options: FileOperationOptions = {}): Promise<boolean> {
    const result = await ErrorHandlingUtils.executeWithErrorHandling(
      async () => {
        const exists = await this.exists(uri);
   ...
   * 搜索文件
   */
  static async findFiles(pattern: string, options: FileSearchOptions = {}): Promise<vscode.Uri[]> {
    const result = await ErrorHandlingUtils.executeWithErrorHandling(
      async () => {
        const includePattern = Array.isArray(options.include) ? `{${options.include.join(',')}}` : options.include;

        const excludePattern = Array.isArray(options.exclude) ? `{${options.exclude.join(',')}}` : options.exclude;

        return vscode.workspace.findFiles(pattern, excludePattern, options.maxResults);
      },
      `搜索文件 ${pattern}`,
      { logError: true, showToUser: false }
    );
   ...
   * 獲取目錄下所有文件
   */
  static async getDirectoryFiles(dirUri: vscode.Uri, recursive: boolean = false): Promise<FileInfo[]> {
    const result = await ErrorHandlingUtils.executeWithErrorHandling(
      async () => {
   ...
   * 遞歸收集文件
   */
  private static async collectFiles(dirUri: vscode.Uri, files: FileInfo[], recursive: boolean): Promise<void> {
  ...
  static async getDirectorySize(dirUri: vscode.Uri): Promise<number> {
    const result = await ErrorHandlingUtils.executeWithErrorHandling(
      async () => {
        ...
        return files.filter(f => f.type === 'file').reduce((total, file) => total + file.size, 0);
      },
