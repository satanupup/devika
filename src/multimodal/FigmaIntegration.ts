...
export class FigmaIntegration {
     ...
     * 生成代碼建議
     */
    async generateCodeFromFigma(fileKey: string, nodeId?: string): Promise<{
        html: string;
        css: string;
        react: string;
        vue: string;
    ...
    private generateCSS(node: FigmaNode): string {
        ...
        if (node.fills && node.fills.length > 0) {
            ...
            if (fill.type === 'SOLID' && fill.color) {
                ...
                styles.push(`background-color: rgba(${Math.round(r * 255)}, ${Math.round(g * 255)}, ${Math.round(b * 255)}, ${a || 1});`);
            }
        }
