...
import { MultimodalProcessor, MediaContent, MediaType, FigmaImportOptions } from './MultimodalProcessor';
...
export class MultimodalCommands {
  ...
  private registerCommands(): void {
    // 截圖相關命令
    this.disposables.push(vscode.commands.registerCommand('devika.takeScreenshot', () => this.takeScreenshot()));

    this.disposables.push(vscode.commands.registerCommand('devika.captureRegion', () => this.captureRegion()));

    this.disposables.push(vscode.commands.registerCommand('devika.captureVSCode', () => this.captureVSCode()));

    // Figma 相關命令
    this.disposables.push(vscode.commands.registerCommand('devika.importFromFigma', () => this.importFromFigma()));

    this.disposables.push(vscode.commands.registerCommand('devika.setFigmaToken', () => this.setFigmaToken()));
    ...
    this.disposables.push(
      vscode.commands.registerCommand('devika.generateCodeFromImage', (uri?: vscode.Uri) =>
        this.generateCodeFromImage(uri)
      )
    );
    ...
    this.disposables.push(vscode.commands.registerCommand('devika.clearMultimodalCache', () => this.clearCache()));
  }
  ...
  private async showCaptureResult(uri: vscode.Uri, message: string): Promise<void> {
    const action = await vscode.window.showInformationMessage(message, '查看圖像', '分析圖像', '生成代碼');
  ...
  private async showAnalysisResults(mediaContent: MediaContent): Promise<void> {
    const panel = vscode.window.createWebviewPanel('imageAnalysis', '圖像分析結果', vscode.ViewColumn.Two, {
      enableScripts: true
    });
  ...
  private async showCodeSuggestions(components: ComponentIdentification[], suggestions: string[]): Promise<void> {
    const panel = vscode.window.createWebviewPanel('codeSuggestions', '代碼建議', vscode.ViewColumn.Two, {
      enableScripts: true
    });
  ...
  private async showDesignSystemAnalysis(designSystem: DesignSystemInfo): Promise<void> {
    const panel = vscode.window.createWebviewPanel('designSystem', '設計系統分析', vscode.ViewColumn.Two, {
      enableScripts: true
    });
