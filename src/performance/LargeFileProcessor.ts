        ...
        return new Promise<StreamProcessingResult<T[]>>((resolve, reject) => {
          stream.on('data', async (chunk: string | Buffer) => {
            try {
              ...
              const chunkString = chunk.toString(opts.encoding);

              // 處理塊
              const result = await processor(chunkString, chunkIndex++);
              if (result !== null) {
                results.push(result);
              }

              processedBytes += Buffer.byteLength(chunkString, opts.encoding);
