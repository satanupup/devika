...
export class WorkspaceEditManager {
    ...
    async findAndReplace(
        ...
        try {
            ...
            // 執行搜索
            const searchResults = await vscode.workspace.findTextInFiles(query, {
            ...
            for (const [uri, matches] of searchResults) {
                try {
                    ...
                    for (const match of matches) {
                        const newText = options.useRegex && searchPattern instanceof RegExp
     ...
     * 驗證和排序操作
     */
    private validateAndSortOperations(
        operations: EditOperation[],
