...
export class FileSystemOptimizer {
   ...
   * 優化的文件掃描
   */
  async scanFiles(workspaceFolder: vscode.WorkspaceFolder, options: ScanOptions = {}): Promise<ScanResult> {
    ...
    try {
      ...
      const foundFiles = await vscode.workspace.findFiles(includePattern, excludePattern, maxFiles);

      // 並行處理文件信息
      const batches = this.createBatches(foundFiles, batchSize);
      const concurrencyLimit = Math.min(maxConcurrency, batches.length);

      await this.processBatchesConcurrently(batches, concurrencyLimit, async batch => {
        const batchResults = await Promise.allSettled(batch.map(uri => this.getFileInfo(uri, workspaceFolder.uri)));
  ...
  async batchOperation<T>(
    ...
    const startTime = Date.now();
    const { batchSize = 50, maxConcurrency = 3, onProgress, continueOnError = true } = options;
    ...
    await this.processBatchesConcurrently(batches, maxConcurrency, async batch => {
      ...
      for (const result of batchResults) {
        if (result.status === 'fulfilled') {
          const opValue = result.value;
          if (opValue.success) {
            successful.push(opValue.result);
          } else {
            failed.push({ file: opValue.file, error: opValue.error });
