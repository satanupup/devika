...
export class TerminalIntegrationSystem {
    ...
    async executeTemplate(templateId: string, sessionId?: string): Promise<TerminalCommand[]> {
        ...
        // 創建新會話或使用現有會話
        const actualSessionId = sessionId || await this.createTerminalSession(
            template.name,
            template.type,
            {
     ...
     * 運行構建
     */
    async runBuild(configName: string, target: 'development' | 'production' | 'test' = 'development'): Promise<TerminalCommand[]> {
    ...
    private initializeDefaultTemplates(): void {
        ...
        this.templates.set('react-dev', {
            ...
            type: 'build',
            commands: [
                { name: 'start', command: 'npm start', description: '啟動開發服務器', order: 1 }
            ],
