...
export class TypeValidator {
  ...
  static validateConfig(value: unknown): value is StrictConfig {
    ...
    if (
      !TypeGuards.hasPropertyOfType(prefs, 'preferredModel', TypeGuards.isString) ||
    ...
    if (
      !TypeGuards.hasPropertyOfType(perf, 'cacheSize', TypeGuards.isPositiveNumber) ||
  ...
  static validateApiResponse<T>(
    ...
    if (value.success) {
      ...
      if (
        !TypeGuards.hasPropertyOfType(error, 'code', TypeGuards.isNonEmptyString) ||
 ...
 * 運行時類型檢查裝飾器
 */
export function validateParams<T extends any[]>(...validators: { [K in keyof T]: (value: unknown) => value is T[K] }) {
