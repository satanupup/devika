...
export class TaskTreeItem extends vscode.TreeItem {
  ...
  private getDescription(): string {
    ...
    if (this.task.subtasks && this.task.subtasks.length > 0) {
        const completedSubtasks = this.task.subtasks
            .map(id => this.taskManager.getTask(id))
            .filter(t => t && t.status === 'completed').length;
...
export class TaskTreeProvider implements vscode.TreeDataProvider<TaskTreeItem> {
  ...
  getChildren(element?: TaskTreeItem): Thenable<TaskTreeItem[]> {
    if (!element) {
      ...
      // 返回子任務
      return Promise.resolve(this.getSubTaskItems(element));
    }
  }
  ...
  private loadTasks(): void {
    ...
    this.rootTasks = allTasks.filter(task => {
      return !allTasks.some(otherTask => otherTask.subtasks?.includes(task.id));
    });
  }
  ...
  private getSubTaskItems(parentItem: TaskTreeItem): TaskTreeItem[] {
      const parentTask = parentItem.task;
      ...
      return subtasks.map(task => this.createTaskItem(task, parentItem));
  }
  ...
  private createTaskItem(task: Task, parent?: TaskTreeItem): TaskTreeItem {
    ...
    return new TaskTreeItem(task, collapsibleState, this.taskManager, parent);
  }
  ...
  async addTask(parentTask?: Task): Promise<void> {
    ...
    if (parentTask) {
      ...
      await this.taskManager.updateTask(parentTask.id, { subtasks: updatedSubtasks });
    }
